...
#
# Pushing the tag triggers .github/workflows/publish-nuget-clinicalreference.yml: packs
# OpenCdsi.ClinicalReference (version auto-derived from this same tag via MinVer - nothing to
# keep in sync by hand) and publishes it to GitHub Packages.
#
# This repo is a monorepo with several independently-tagged solutions (engine-v*, mobile-v*,
# backend-v*, clinref-v*, ...). This script only ever cuts clinref-v* releases - takes the bare
# vX.Y.Z version and adds the "clinref-" prefix itself, matching release-tag-engine.sh/
# release-tag-mobile.sh/release-tag-backend.sh's own pattern.
set -euo pipefail
