...
#
# Pushing the tag triggers .github/workflows/publish-nuget.yml: packs OpenCdsi.VaxEngine.Core
# (version auto-derived from this same tag via MinVer - nothing to keep in sync by hand) and
# publishes it to GitHub Packages.
#
# This repo is a monorepo with several independently-tagged solutions (engine-v*, mobile-v*,
# ...). This script only ever cuts engine-v* releases - takes the bare vX.Y.Z version and adds
# the "engine-" prefix itself, matching release-tag-mobile.sh's own pattern.
set -euo pipefail
