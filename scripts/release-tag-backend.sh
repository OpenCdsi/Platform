...
#
# Pushing the tag triggers .github/workflows/release-backend.yml: builds the multi-arch container
# image of src/OpenCdsi.VaxEngine.Api and pushes it to ghcr.io/<owner>/platform/vaxengine-api.
#
# This repo is a monorepo with several independently-tagged solutions (engine-v*, mobile-v*,
# backend-v*, ...). This script only ever cuts backend-v* releases - takes the bare vX.Y.Z
# version and adds the "backend-" prefix itself, matching release-tag-engine.sh/
# release-tag-mobile.sh's own pattern.
set -euo pipefail
